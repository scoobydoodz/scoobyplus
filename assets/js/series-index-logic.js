...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!seriesId || typeof CONTENT_CATEGORIES === 'undefined') {
        document.getElementById('series-title').textContent = 'Erro: Dados não encontrados.';
        return;
    }
    ...
    const title = foundItem.title || (tmdbData ? (tmdbData.name || tmdbData.title) : 'Título não disponível');
    const overview = foundItem.overview || (tmdbData ? tmdbData.overview : 'Sinopse não disponível.');
    const posterPath = foundItem.poster_path || 
        (tmdbData && tmdbData.poster_path ? `${TMDB_IMAGE_BASE}${tmdbData.poster_path}` : 'https://via.placeholder.com/250x350/1e1e1e/FFFFFF?text=SEM+POSTER');
    const backdropPath = foundItem.backdrop_path || 
        (tmdbData && tmdbData.backdrop_path ? `${TMDB_BACKDROP_BASE}${tmdbData.backdrop_path}` : '');
    const releaseDate = tmdbData ? (tmdbData.first_air_date || tmdbData.release_date) : null;
    const year = foundItem.year || (releaseDate ? releaseDate.substring(0, 4) : 'Clássico');
    ...
    if (isSeries) {
        ...
        document.getElementById('metadata').innerHTML = `<span>${year}</span> | <span>${seasonsList.length} Temporadas</span>`;
        ...
        async function loadEpisodes(seasonNumber) {
            ...
            listElement.innerHTML = '<li class="loading-item" style="padding: 15px 25px;">Carregando episódios...</li>';
            
            // Busca dados dos episódios do TMDB se disponível
            let tmdbEpisodes = null;
            if (isValidTmdbId) {
                tmdbEpisodes = await fetchTmdbData(`tv/${seriesId}/season/${seasonNumber}`);
            }
            ...
            localEpisodes.forEach(ep => {
                ...
                if ((!ep.title || ep.title === `Episódio ${ep.number}`) && tmdbEpisodes && tmdbEpisodes.episodes) {
                    const tmdbEp = tmdbEpisodes.episodes.find(e => e.episode_number === ep.number);
                    if (tmdbEp) {
        ...
        let lastSeason = 1, lastEpisode = 1;
        ...
        document.getElementById('metadata').innerHTML = `<span>${year}</span> | <span>Filme</span>`;
        ...
        if (videoId) {
            ...
            if (videoId.startsWith('gdrive:')) {
                ...
                playerHtml = `
                    <div class="movie-player-section">
                        ...
                        <div class="dailymotion-embed-wrapper">
                            <iframe 
                                frameborder="0" 
                                width="100%" 
                                height="500" 
            ...
            document.getElementById('episodes-list').innerHTML = '<div class="movie-player-section"><h3>Filme Indisponível</h3><p>Este filme ainda não está disponível para assistir.</p></div>';
        }
        ...
        continueBtn.addEventListener('click', (e) => {
            e.preventDefault();
            document.getElementById('episodes-list').scrollIntoView({ behavior: 'smooth' });
        });
    }
});